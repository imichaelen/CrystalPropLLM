...
def round_numbers(cif_str, decimal_places=4):
    # Pattern to match floating point numbers (must contain a decimal point or scientific notation)
    # This excludes pure integers
    pattern = r"[-+]?(?:\d*\.\d+(?:[eE][-+]?\d+)?|\d+\.\d*(?:[eE][-+]?\d+)?|\d+[eE][-+]?\d+)"
