#!/usr/bin/env python3
"""
Training Script for CrystalPropLLM.

Train GPT or DeepSeek MLA models for crystal structure and property prediction.

Workflow:
1. Load tokenized training and validation data
2. Initialize or resume model from checkpoint
3. Train model with AdamW optimizer
4. Evaluate on validation set periodically
5. Save checkpoints and log metrics to WandB
...
Usage:
  ...
  Resume training from checkpoint:
    python train.py config.yaml --init_from=resume
  
  Training with custom parameters:
    python train.py config.yaml --learning_rate=1e-4 --max_iters=100000
...
from crystalpropllm import parse_config
...
class TrainDefaults:
    ...
    model_type: str = "gpt2"  # "gpt2" or "deepseek" model architecture
    n_block_types: int = 4  # number of different block types for block embeddings
    # DeepSeek MLA specific parameters (only used when model_type="deepseek")
    ...
    wandb_project: str = "crystalpropllm"  # wandb project name
    wandb_run_name: str = "mp_prop"  # wandb run name, e.g., 'run' + str(time.time())
    wandb_run_id: Optional[str] = None  # wandb run id for resuming, if None will create new run
    wandb_resume: str = "allow"  # "allow", "must", "never", or "auto"
...
if __name__ == "__main__":
    ...
    # Initialize wandb with proper resume handling
    if C.wandb_log:
        wandb_config = {
            "project": C.wandb_project,
            "name": C.wandb_run_name,
            "config": OmegaConf.to_container(C, resolve=True, throw_on_missing=True)
        }
        
        # Handle manual run ID for resuming
        if C.wandb_run_id:
            wandb_config["id"] = C.wandb_run_id
            wandb_config["resume"] = C.wandb_resume
            print(f"Using provided wandb run ID: {C.wandb_run_id} with resume mode: {C.wandb_resume}")
    ...
    train_block_ids = np.memmap(os.path.join(C.dataset, "train_block_ids.bin"), dtype=np.uint16, mode="r")
    val_block_ids = np.memmap(os.path.join(C.dataset, "val_block_ids.bin"), dtype=np.uint16, mode="r") if C.validate else None
    ...
    def get_batch(split):
        ...
        block_ids_data = train_block_ids if split == "train" else val_block_ids
        ...
        b = torch.stack([torch.from_numpy((block_ids_data[i:i + C.block_size]).astype(np.int64)) for i in ix])

        if device_type == "cuda":
            # pin arrays x,y, which allows us to move them to GPU asynchronously (non_blocking=True)
            x, y, b = x.pin_memory().to(C.device, non_blocking=True), y.pin_memory().to(C.device, non_blocking=True), b.pin_memory().to(C.device, non_blocking=True)
        else:
            x, y, b = x.to(C.device), y.to(C.device), b.to(C.device)
        return x, y, b
    ...
    model_args = dict(n_layer=C.n_layer, n_head=C.n_head, n_embd=C.n_embd, block_size=C.block_size,
                      bias=C.bias, vocab_size=None, dropout=C.dropout, model_type=C.model_type,
                      n_block_types=C.n_block_types,
                      d_kv=C.d_kv, d_k_low=C.d_k_low, d_v_low=C.d_v_low, d_k_high=C.d_k_high, d_v_high=C.d_v_high)
    if C.init_from == "scratch":
        ...
        # Handle n_block_types with backward compatibility for old checkpoints
        if "n_block_types" in checkpoint_model_args:
            model_args["n_block_types"] = checkpoint_model_args["n_block_types"]
        else:
            print(f"n_block_types not found in checkpoint, using default value: {C.n_block_types}")
            # Keep the default value from TrainDefaults
        # Also load DeepSeek MLA parameters if present
        for k in ["d_kv", "d_k_low", "d_v_low", "d_k_high", "d_v_high"]:
    ...
    scaler = torch.amp.GradScaler('cuda', enabled=(C.dtype == "float16"))
    ...
    def estimate_loss():
        ...
        for split, eval_iters in [("train", C.eval_iters_train), ("val", C.eval_iters_val)]:
            ...
            for k in range(eval_iters):
                X, Y, B = get_batch(split)
                with ctx:
                    logits, loss = model(X, B, Y)
                losses[k] = loss.item()
    ...
    while True:
        ...
        if iter_num % C.eval_interval == 0:
            if C.validate:
                ...
                if C.wandb_log:
                    wandb.log({
                        "iter": iter_num, # Log iter to align with other metrics
                        "val_loss": losses['val'],
                        "train_eval_loss": losses['train'],
                    })
            old_best_val_loss = best_val_loss # Store best_val_loss before potential update
            if (C.validate and losses["val"] < best_val_loss) or C.always_save_checkpoint:
                best_val_loss = losses["val"] if C.validate else 0.
                if C.wandb_log and C.validate and best_val_loss < old_best_val_loss:
                    # Log the new best validation loss if it improved
                    wandb.log({"iter": iter_num, "best_val_loss": best_val_loss})
                if iter_num > 0:
                    checkpoint = {
