#!/usr/bin/env python3
"""
Preprocessing Script with Changed Block Ordering for CrystalPropLLM.

Preprocess CIF files with Operation → Structure → Target block ordering.

Workflow:
1. Load CIF data from pickle file
2. Auto-detect and load properties.json (no features)
3. Process CIFs in order: OPERATION_ → structure → TARGET_
4. Generate vocabulary from property/condition keys
5. Save preprocessed CIFs to pickle file

Usage:
  Basic preprocessing:
    python preprocess_change_order.py data.pkl.gz
  
  With custom output:
    python preprocess_change_order.py data.pkl.gz --out preprocessed.pkl.gz
  
  With oxidation states:
    python preprocess_change_order.py data.pkl.gz --oxi
"""
...
def augment_cif(progress_queue, task_queue, result_queue, oxi, decimal_places, data, feature_order=None, skip_features=False):
    ...
    while not task_queue.empty():
        ...
        try:
            ...
            # Process the CIF structure first
            ...
            # Now build the final output in the order: Operation → Structure → Target
            output_parts = []
            
            # 1. Add OPERATION_ block first
            op_conditions = row_data.get('operation_conditions', {})
            if op_conditions:
                operation_lines = ["OPERATION_"]
                for key, value in op_conditions.items():
                    if value is not None:
                        operation_lines.append(f"_opera_{key} {value}")
                output_parts.append("\n".join(operation_lines))
            
            # 2. Add the CIF structure data second
            output_parts.append(cif_str.rstrip())
            
            # 3. Add TARGET_ block last
            ...
            # Combine all parts without extra blank lines between blocks
            final_output = "\n".join(output_parts)
            
            # Apply rounding to the entire final output
            final_output = round_numbers(final_output, decimal_places=decimal_places)
            
            augmented_cifs.append((composite_id, final_output))
...
if __name__ == "__main__":
    ...
    # Force skip features - we don't want feature blocks in this version
    skip_features = True
    print("Skipping feature blocks (disabled in this version)")
