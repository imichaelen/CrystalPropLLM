#!/usr/bin/env python3
"""
Comprehensive Evaluation Script for CrystalPropLLM.

Evaluate trained models on test data with complete pipeline.

Workflow:
1. Generate prompts from preprocessed test data
...
Usage:
  Basic evaluation with input folder:
    python evaluate.py --input_folder /path/to/data/ --model /path/to/model/
  
  With custom test file:
    python evaluate.py --test_file test.pkl.gz --model /path/to/model/ --output_dir results/
  
  Custom generation parameters:
    python evaluate.py --input_folder /path/to/data/ --model /path/to/model/ --num_gens 5 --temperature 0.8
"""
...
# Regex patterns for prompt extraction
PATTERN_COMP = re.compile(r"(data_[^\n]*\n)", re.MULTILINE)
PATTERN_COMP_SG = re.compile(r"(data_[^\n]*\n)loop_[\s\S]*?(_symmetry_space_group_name_H-M[^\n]*\n)", re.MULTILINE)
PATTERN_COMP_PROP = re.compile(r"(data_[^\n]*\n)loop_[\s\S]*?(TARGET_)", re.MULTILINE)
PATTERN_COMP_SG_PROP = re.compile(r"(data_[^\n]*\n)loop_[\s\S]*?(_symmetry_space_group_name_H-M[^\n]*\n)[\s\S]*?(TARGET_)", re.MULTILINE)
...
def extract_prompt(cif_str, pattern):
    """Extract prompts from CIF string using regex pattern."""
    match = re.search(pattern, cif_str)
    if match:
        start_index = match.start()
        if pattern in [PATTERN_COMP_PROP, PATTERN_COMP_SG_PROP]:
            # For property patterns, include just the property name
            groups = match.groups()
            if len(groups) == 2:  # PATTERN_COMP_PROP
                cif_str = groups[0] + match.group(0)[len(groups[0]):match.end(2)] + '\n'
            elif len(groups) == 3:  # PATTERN_COMP_SG_PROP
                middle_part = match.group(0)[match.end(1):match.start(3)]
                cif_str = groups[0] + middle_part + groups[2] + '\n'
        else:
...
def generate_prompts(test_file, output_dir, with_spacegroup=False, with_property=True):
    ...
    if with_spacegroup and with_property:
        pattern = PATTERN_COMP_SG_PROP
    elif with_property:
        pattern = PATTERN_COMP_PROP
...
def create_summary_log(all_metrics, output_dir):
    ...
    with open(log_file, 'w') as f:
        f.write(f"CrystalPropLLM Evaluation Summary\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Comprehensive evaluation of CrystalPropLLM on test data."
    )
    ...
    parser.add_argument("--with_property", action="store_true", default=True,
                        help="Include property target in prompts")
    ...
    try:
        ...
        prompts_file = generate_prompts(
            ...
            with_property=args.with_property
        )
